...
class KeywordProcessor(object):
    ...
    def __init__(self, case_sensitive=False, multi_keywords=True):
        """
        Args:
        ...
        self.non_word_boundary_pattern = re.compile('[a-z0-9A-Z\u4e00-\u9fa5]')
    ...
    def __setitem__(self, keyword, clean_name=None):
        ...
        """
        status = False
        # 如果没有设置clean_name，且keyword不为None
        # 设定
        ...
        if keyword and clean_name:
            ...
            for letter in keyword:
                # 如果字典存在 key，letter 存在，则会让current_dict = currenct_dict[letter]
                # 若不存在，则让currenct_dict[letter] = {}， 同时让current_dict =
                current_dict = current_dict.setdefault(letter, {})
            if self.multi_keywords is False:
    ...
    def set_non_word_boundary_pattern(self, boundary_pattern):
        self.non_word_boundary_pattern = re.compile('[a-z0-9A-Z\u4e00-\u9fa5]')
    ...
    def is_word_boundary(self, char):
        if char not in self.non_word_boundaries and self.non_word_boundary_pattern.match(char) is None:
            return True
        else:
            return False
    ...
    def extract_keywords(self, sentence, span_info=False):
        ...
        while idx < sentence_len:
            char = sentence[idx]
            # when we reach a character that might denote word end, 也就是说可能是word end
            # and self.non_word_boundary_pattern.match(char) is None
    ...
    def replace(self, sentence):
        res = self.extract_keywords(sentence, span_info=True)
        if len(res) == 0:
            return sentence
        else:
            components = []
            ...
            for i, v in enumerate(res):
                word, pos1, pos2 = v
                if self.multi_keywords is True:
                    word = random.choice(list(word))
                # 第一个的处理
                if i == 0:
                    components.append(sentence[:pos1])
                    components.append(word)
                    if i == k_num - 1:
                        components.append(sentence[pos2:])
                # 最后一个的处理方法也不一样
                elif i == k_num - 1:
                    components.append(sentence[last_pos:pos1])
                    components.append(word)
                    components.append(sentence[pos2:])
                else:
                    components.append(sentence[last_pos:pos1])
    ...
    def append(self, sentence, rep_symbol='@'):
        res = self.extract_keywords(sentence, span_info=True)
        if len(res) == 0:
            return sentence
        else:
            for v in res:
                words, pos1, pos2 = v
                for word in words:
                    sentence += ' ' + rep_symbol + word
        return sentence


if __name__ == '__main__':
    kp = KeywordProcessor()
    kp.add_keyword('长沙市', '湖南')
